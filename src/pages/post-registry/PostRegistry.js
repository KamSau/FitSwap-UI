...
export default function PostRegistry({history}) {
  ...
  useEffect(() => {
    ...
    if (pressed == 1) {
		...
		let post = {url: purl, title: ptitle, description: pdescription, userId: 1}
		console.log(post);
		let valid = false;
		  valid = validate(post);
			if(valid){
			Axios.post("http://localhost:5000/api/v1/post", post).then((res) => {
  ...
  let validate = (user) => {
    let valid = false;
      if(user.url === "" || user.title === "" || user.description === "" || pressed == 0){
		valid = false;
		setPressed(0);
      }else{
        valid = true;
